...
The surface here is the one the coursework brief specifies for `GradeBook`: averages,
pass rates, regex search, and JSON/CSV interchange. The application does not call it —
`services/` computes the same quantities in SQL — but the core is graded on its own
terms and stays exercised on its own terms.
"""
...
def _empty_store() -> GradeStore:
    """A second, independent store.

    The `store` fixture cannot serve here: `gradebook` is built on it, so a test that
    needs both a populated book and an empty one would get the same rows twice.
    """
    conn = connect(":memory:")
    apply_migrations(conn)
    return GradeStore(conn)
...
class TestJsonInterchange:
    def test_round_trip_preserves_everything(self, gradebook: GradeBook, tmp_path: Path) -> None:
        ...
        restored = GradeBook(_empty_store())
    ...
    def test_malformed_json_raises_validation(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValidationError):
            GradeBook(_empty_store()).load_json(path)
